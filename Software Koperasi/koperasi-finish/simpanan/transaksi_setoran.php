...
if(isset($_POST['btnSimpan'])){
	...
	if (trim($txtNilaiTransaksi)=="" or ! is_numeric(trim($txtNilaiTransaksi))) {
		$pesanError[] = "Data <b>Setoran Tunai (Rp)</b> masih kosong, harus diisi angka !";
	}
	else {
		if (trim($txtNilaiTransaksi)< 5000 ) {
			$pesanError[] = "Data <b>Nilai Setoran</b> masih kurang, minimal adalah <b> Rp. 5000 </b> !";
		}
	}
	...
	if (count($pesanError)>=1 ){
		...
		$mySql	= "INSERT INTO simpanan_transaksi (no_transaksi, tgl_transaksi, no_simpanan, debit, kredit, keterangan, kd_pegawai) 
					VALUES ('$kodeTransaksi', '$txtTanggal',  '$txtKode', '0', '$txtNilaiTransaksi', '$txtKeterangan', '$userLogin')";
		$myQry	= mysql_query($mySql, $koneksidb) or die ("Gagal query simpanan 1 ".mysql_error());
		if($myQry) {
			// Refresh halaman
			echo "<meta http-equiv='refresh' content='0; url=?open=Transaksi-Setoran&Kode=$txtKode'>";		
		}
		
		// Cetak nota simpanan
		echo "<script>";
		echo "window.open('setoran_nota.php?Kode=$kodeTransaksi')";
		echo "</script>";
	}	
}
...
<form action="<?php $_SERVER['PHP_SELF']; ?>" method="post" name="form1" target="_self">
  <table width="800" cellspacing="1"  class="table-list">
    <tr>
      <td width="21%" bgcolor="#CCCCCC"><strong>DATA SIMPANAN </strong></td>
      <td width="1%">&nbsp;</td>
      <td width="78%">&nbsp;</td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td><strong>:</strong></td>
      <td><input name="txtNomor" value="<?php echo $noTransaksi; ?>" size="22" maxlength="22" disabled="disabled"/>
      <input name="txtKode" type="hidden" value="<?php echo $myData['no_simpanan']; ?>" /></td>
    </tr>
    <tr>
      <td><strong>Tgl.  Transaksi </strong></td>
      <td><strong>:</strong></td>
      <td><input name="txtTanggal" type="text" class="tcal" value="<?php echo $dataTanggal; ?>" size="22" maxlength="22" /></td>
    </tr>
    <tr>
      <td><strong>Setoran Tunai (Rp.) </strong></td>
      <td><strong>:</strong></td>
      <td><b>
        <input name="txtNilaiTransaksi" value="<?php echo $dataNilaiTransaksi; ?>" size="22" maxlength="12"/>
      </b></td>
    </tr>
    <tr>
      <td><strong> Keterangan </strong></td>
